...
public class SapResource {
    ...
    @Path("/load/component/sap")
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public Response loadComponentSap() throws Exception {
        /* This is an autogenerated test */
        if (context.getComponent(COMPONENT_SAP) != null) {
            return Response.ok().build();
        }
        LOG.warnf("Could not load [%s] from the Camel context", COMPONENT_SAP);
        return Response.status(500, COMPONENT_SAP + " could not be loaded from the Camel context").build();
    }
}
